...
class HumanCritic(object):
	...
	def init_tf(self):
		...
		with self.graph.as_default():
			...
			self.loss = -tf.add(tf.multiply(self.preference_distribution[0], tf.math.log(self.p_o0_o1)), \
								tf.multiply(self.preference_distribution[1], tf.math.log(self.p_o1_o0)))
			...
			self.loss = tf.add(1e-5,-tf.add(tf.multiply(self.preference_distribution[0],tf.math.log(self.p_o0_o1)), \
					tf.multiply(self.preference_distribution[1],tf.math.log(self.p_o1_o0))))
      note 2 tf.optimizers.Adam(learning_rate=self.LEARNING_RATE).minimize(self.loss, var_list = [self.p_o0_o1, self.p_o1_o0])
			#'''
		...
		with self.graph.as_default():
			with tf.compat.v1.variable_scope("hp_model"):
				...
				for i in range(self.LAYER_COUNT):
					layer = slim.fully_connected(model, self.NEURON_SIZE, activation_fn=tf.nn.relu,
	...
	def create_model(self, input_data, reuse=False):
		with self.graph.as_default():
			with tf.compat.v1.variable_scope("hp_model"):
				...
				# Programatically define Layers
				for i in range(self.LAYER_COUNT):
					layer = tf.compat.v1.layers.dense(model, self.NEURON_SIZE, activation=tf.nn.relu,
													  kernel_initializer=self.initializer,
													  use_bias=True, name="hp_model_" + str(i), trainable=True,
													  reuse=reuse)
					model = layer

				layer = tf.compat.v1.layers.dense(model, 1, name="output", \
												  reuse=reuse, kernel_initializer=self.initializer, trainable=True,
												  use_bias=False)
		...
		with self.graph.as_default():
			for i in r[:batch_size]:
				...
				with self.graph.as_default():
					...
					feed_dict = {self.input_o0: o0, self.input_o1: o1, self.preference_distribution: pref_dist,
								 self.batch_sizes: [len(o0), len(o1)]}
					m0, m1, loss, _ = self.sess.run([self.model_o0_sum, self.model_o1_sum, self.loss, self.train_step],
													feed_dict=feed_dict)
